...
ModifyHeaders.ExportImport = (function () {
	return {
		...
		init: function () {
			...
			document.getElementById("select-headers-tree").view = ModifyHeaders.ExportImport.selectHeadersTreeView;
		},
		
		selectAllHeaders: function (checkBox) {
			for (var i = 0; i < this.modifyheadersService.count; i++) {
				this.selectedRows[i] = !checkBox.checked;
			}
		...
		headersSelected: function () {
			...
			for (var i = 0; i < ModifyHeaders.ExportImport.modifyheadersService.count; i++) {
				if (ModifyHeaders.ExportImport.selectedRows[i]) {
					trueCount++;
				}
			}
		...
		selectHeadersTreeView: {
	        ...
	        getCellValue: function (row,column) {
	        	if (column == "col-select" || column.id == "col-select") {
	        		return ModifyHeaders.ExportImport.selectedRows[row];
	        	}
	        	return null;
	        },
	        setCellValue: function (row, column, value) {
	        	if (column == "col-select" || column.id == "col-select") {
	        		ModifyHeaders.ExportImport.selectedRows[row] = (value == "false" ? false : true);
	        		ModifyHeaders.ExportImport.headersSelected();
	        	}
	        },
		...
		ExportWizard: {
			...
			init: function () {
				if (!this.initiated) {
					ModifyHeaders.ExportImport.init();
			...
			saveConfiguration: function () {
				...
				try {
		            ...
		            foStream.init(this.theFile, flags, 0664, 0);
